...
class YOLOTwoStageMeterReader:
    ...
    def analyze(
        ...
        # compute tile regions.  Start with a copy of the original ranges.
        ...
        # Compute transformation of dial polygon to get dial bounds in rotated coordinates
        ...
        dial_y_min = float(np.min(transformed[:, 1]))
        dial_y_max = float(np.max(transformed[:, 1]))
        dial_width = max(dial_x_max - dial_x_min, 1.0)
        dial_height = max(dial_y_max - dial_y_min, 1.0)
        # Prepare containers for detection results per tile
        detection_digits: List[str] = [''] * len(self.digit_ranges)
        detection_confs: List[float] = [0.0] * len(self.digit_ranges)
        detection_bboxes: List[Optional[Tuple[float, float, float, float]]] = [None] * len(self.digit_ranges)
        # Stage 4: detect digits on rotated meter (if detector provided)
        if self.digit_detector is not None:
            ...
            # Extract detections and compute ratios relative to dial bounds
            ...
            if ratios_list:
                ...
                # Assign detections to tile indices and record bounding boxes
                for ratio, cls_idx, conf_val, bbox in zip(ratios_list, classes_list, confs_list, boxes_list):
                    # Determine which adjusted range this detection falls into
                    tile_idx = None
                    for idx, (start, end) in enumerate(adjusted_ranges):
                        if start <= ratio <= end:
                            tile_idx = idx
                            break
                    if tile_idx is None:
                        continue
                    # Store detection digit and confidence if higher than previous
                    if detection_bboxes[tile_idx] is None or conf_val > detection_confs[tile_idx]:
                        detection_digits[tile_idx] = str(cls_idx)
                        detection_confs[tile_idx] = conf_val
                        detection_bboxes[tile_idx] = bbox
        # Compute bounding boxes for all 8 digit positions
        tile_bboxes: List[Tuple[int, int, int, int]] = [None] * len(self.digit_ranges)
        # Margin factors (extend bbox by 10% on each side)
        margin_x_factor = 0.10
        margin_y_factor = 0.10
        # Helper to enlarge and clamp a bounding box
        def enlarge_and_clamp(bbox: Tuple[float, float, float, float]) -> Tuple[int, int, int, int]:
            bx1, by1, bx2, by2 = bbox
            bw = bx2 - bx1
            bh = by2 - by1
            mx = bw * margin_x_factor
            my = bh * margin_y_factor
            x1 = int(np.floor(bx1 - mx))
            y1 = int(np.floor(by1 - my))
            x2 = int(np.ceil(bx2 + mx))
            y2 = int(np.ceil(by2 + my))
            # Clamp to dial bounds (optionally allow outside dial bounds)
            x1 = max(int(dial_x_min), min(x1, int(dial_x_max)))
            y1 = max(int(dial_y_min), min(y1, int(dial_y_max)))
            x2 = max(int(dial_x_min), min(x2, int(dial_x_max)))
            y2 = max(int(dial_y_min), min(y2, int(dial_y_max)))
            ...
            if y2 <= y1:
                y2 = y1 + 1
            return x1, y1, x2, y2
        # First, assign detected bboxes (with margin)
        for i in range(len(self.digit_ranges)):
            if detection_bboxes[i] is not None:
                tile_bboxes[i] = enlarge_and_clamp(detection_bboxes[i])
        # Now compute missing tile boxes
        for i in range(len(self.digit_ranges)):
            if tile_bboxes[i] is not None:
                continue
            # Find nearest left and right neighbor indices that have bounding boxes
            left_idx = None
            right_idx = None
            # search left
            for j in range(i - 1, -1, -1):
                if tile_bboxes[j] is not None:
                    left_idx = j
                    break
            # search right
            for k in range(i + 1, len(self.digit_ranges)):
                if tile_bboxes[k] is not None:
                    right_idx = k
                    break
            # Compute bounding box based on neighbors or predicted ratios
            if left_idx is not None and right_idx is not None:
                # Use neighbor centers to estimate this tile's centre and width
                lx1, ly1, lx2, ly2 = tile_bboxes[left_idx]
                rx1, ry1, rx2, ry2 = tile_bboxes[right_idx]
                left_center_x = (lx1 + lx2) / 2.0
                right_center_x = (rx1 + rx2) / 2.0
                # width between neighbour centres
                width_i = right_center_x - left_center_x
                x_center = (left_center_x + right_center_x) / 2.0
                x1 = int(np.floor(x_center - width_i / 2.0))
                x2 = int(np.ceil(x_center + width_i / 2.0))
                # Use dial vertical bounds for y
                y1 = int(dial_y_min)
                y2 = int(dial_y_max)
                tile_bboxes[i] = enlarge_and_clamp((x1, y1, x2, y2))
            else:
                # Use predicted range from adjusted_ranges
                start, end = adjusted_ranges[i]
                # Predicted bounding box using dial bounds
                px1 = dial_x_min + start * dial_width
                px2 = dial_x_min + end * dial_width
                x1 = int(np.floor(px1))
                x2 = int(np.ceil(px2))
                y1 = int(dial_y_min)
                y2 = int(dial_y_max)
                tile_bboxes[i] = enlarge_and_clamp((x1, y1, x2, y2))
        # Crop each tile from rotated_meter using computed bounding boxes
        tiles_cropped: List[np.ndarray] = []
        for bbox in tile_bboxes:
            x1, y1, x2, y2 = bbox
            # Clamp bounding box to image bounds
            x1_c = max(0, min(x1, rotated_meter.shape[1] - 1))
            x2_c = max(0, min(x2, rotated_meter.shape[1]))
            y1_c = max(0, min(y1, rotated_meter.shape[0] - 1))
            y2_c = max(0, min(y2, rotated_meter.shape[0]))
            if x2_c <= x1_c or y2_c <= y1_c:
                # create a blank small tile if invalid
                tile_img = rotated_meter[int(dial_y_min):int(dial_y_min)+1, int(dial_x_min):int(dial_x_min)+1].copy()
            else:
                tile_img = rotated_meter[y1_c:y2_c, x1_c:x2_c].copy()
            # Optionally resize to 28x28 for classification
            resized_tile = cv2.resize(tile_img, (28, 28), interpolation=cv2.INTER_LANCZOS4)
            tiles_cropped.append(resized_tile)
        # Classify each tile using the digit classifier (if provided)
        classification_digits: List[str] = [''] * len(tiles_cropped)
        classification_confs: List[float] = [0.0] * len(tiles_cropped)
        if self.classifier is not None:
            classification_digits, classification_confs = self.classify_digits(tiles_cropped)
        ...
        # Compute a sharpened dial image for returning and logging
        ...
        return pred_str, sharpened_dial, tiles_cropped, meter_crop, dial_crop
