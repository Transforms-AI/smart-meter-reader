...
class YOLOTwoStageMeterReader:
    ...
    def assign_digits_from_ratios(
        self, ratios: np.ndarray, classes: np.ndarray, confs: np.ndarray
    ) -> List[str]:
        """Assign detected digits to positions using precomputed x‑axis ratios.

        This helper takes a list of x‑axis percentage values and maps each
        detection to one of the eight digit positions defined in
        ``self.digit_ranges``.  When multiple detections fall into the same
        range the one with the highest confidence is selected.

        Args:
            ratios: 1‑D array of x‑axis positions as percentages (0–100).
            classes: 1‑D array of class indices (0–9).
            confs: 1‑D array of confidence scores (same length as ratios).

        Returns:
            A list of eight strings where each element corresponds to the
            detected digit for that position, or an empty string if no
            detection was assigned.
        """
        positions: List[Tuple[str, float] | str] = [''] * len(self.digit_ranges)
        for ratio, cls, conf in zip(ratios, classes, confs):
            # Skip any detections outside the normalised range [0, 1]
            if ratio < 0.0 or ratio > 1.0:
                continue
            for idx, (start_pct, end_pct) in enumerate(self.digit_ranges):
                # digit_ranges are normalised, so compare directly
    ...
    def analyze(
        ...
        # Prepare digit tiles for classification fallback
        tiles = self.slice_digits_by_ratio(sharpened_dial, self.digit_ranges, img_width=28, img_height=28)
        # Initialise positions for eight digits
        final_positions: List[str] = [''] * len(self.digit_ranges)
        # Only proceed with detection if a digit detector is provided
        if self.digit_detector is not None:
            ...
            pts = np.array(poly, dtype=np.float32).reshape(-1, 2)  # shape (4, 2)
            pts_h = np.hstack([pts, np.ones((pts.shape[0], 1), dtype=np.float32)])
            transformed = (M @ pts_h.T).T  # shape (4,2)
            dial_x_min = float(np.min(transformed[:, 0]))
            dial_x_max = float(np.max(transformed[:, 0]))
            dial_width = max(dial_x_max - dial_x_min, 1.0)  # avoid division by zero
            # Detect digits on the rotated meter
            ...
            if results_digit and results_digit[0].boxes is not None:
                ...
                # Collect ratios for each detection
                ratios_list: List[float] = []
                classes_list: List[int] = []
                confs_list: List[float] = []
                for i in range(len(boxes)):
                    x1, y1, x2, y2 = boxes[i]
                    print(classes[i],confs[i],boxes[i])
                    x_center = float((x1 + x2) / 2.0)
                    # Normalise x relative to dial bounds
                    ratio = (x_center - dial_x_min) / dial_width
                    # Only consider detections within 0–1 range
                    if 0.0 <= ratio <= 1.0:
                    # ratio is already normalised to [0, 1] relative to dial width
                ...
                if ratios_list:
                    ...
                    detection_positions = self.assign_digits_from_ratios(ratios_arr, classes_arr, confs_arr)
                    final_positions = detection_positions
        # Fallback to classification for missing positions or if no detector
        if self.classifier is not None:
            need_classification = (
                self.digit_detector is None
                or any(pos == '' for pos in final_positions)
            )
            if need_classification:
                pred_cls_str = self.classify_digits(tiles)
                for idx, digit in enumerate(final_positions):
                    if digit == '' and idx < len(pred_cls_str):
                        final_positions[idx] = pred_cls_str[idx]
        # Construct the final string (fill missing digits with '')
        pred_str = ''.join([d if d else '' for d in final_positions])
